...
对照的是 `em-qc-platform` 的 HEAD `607e17b`，工作区是干净的。下面列的每条缺口都附了 file:line。关键结论我逐条翻代码核对过，包括键盘分派、笔画、撤销、填充、修缮、修补、批量删除确认框、工作副本存储顺序、试用登录的默认角色、只读缓存、EM 响应头。文中凡写「实测」的耗时数字，都是审计员测的，我没有重测。
...
- 现在的体验：
  - 按着左键时，滚轮和 A/Z 照样翻片。`goZ` 只拦 `mergeBusy`（`annotate.js:401-402`），`onWheel` 不看 `S.stroke`（`annotate.js:416-421`）。所以墨迹画在新片的画布上，存盘却用落笔时的 `st.z`（`annotate.js:526,562`）。
...
- 建议改法：
  ...
  - `S.stroke` 存在期间，`goZ` 一律拒绝（滚轮、键盘、滑条、连播都走这里）。
  - Esc 丢弃当前这一笔。
...
**4. 修补预览还在算的时候翻片，结果会串到别的片上**（工作量：小）
- 现在的体验：
  - `repairPreview` 没有序号，也没有 abort（`annotate.js:625-638`）。翻片时 `clearRepair` 发现 `S.repair` 还是空的，直接 return（`annotate.js:704-705`）。
  - 等响应回来，旧片的绿色叠加被画到新片上，「应用」按钮也亮了。
  - `repairApply` 用 `S.z` 做 afterEdit（`annotate.js:640-645`），服务端却按 token 写旧片（`interpolate.py:404`）。
- 参考工具：我们自己的 SAM 预览已经有 AbortController 加序号的防护（`annotate.js:727-772`）。
- 建议改法：照 `predictSAM` 的写法加 `S.repairSeq` 和 abort。响应回来时核对 `r.z===S.z`。apply 时用 `S.repair.z`。
...
- 现在的体验：
  ...
  - 修缮、合并、修补这几条路径都有「没改动」的提示（`annotate.js:448,512,646`）。
...
**15. 修缮 R 一点就落盘；SAM 和修补预览没有 Enter 确认**（工作量：小）
- 现在的体验：
  ...
  - 键盘处理里没有 Enter 分支，SAM 和修补都只能点侧栏按钮（`annotate.js:830-850,639-652`）。
...
- 建议改法：
  ...
  - Enter 应用：有 SAM 预览时填为当前标签，有修补预览时应用修补；Shift+Enter 填为新标签。
  - 删掉 `reach`。
...
- 现在的体验：
  ...
  - 扫描损坏同步遍历整块（`annotate.py:350-362`），不能停，也没有进度。
...
- 现在的体验：
  ...
  - 帮助里没有 PageUp/PageDown、Home/End、+/-、空格、Esc（`annotate.html:142-170`）。
  - `annotate_guide.html:21` 还写着「右键拖动平移」，全文没有 A/Z、修缮、全部撤销。
...
**28. 历史列表不能点，也看不到某一笔改了哪里**（工作量：中）
- 现在的体验：历史行是纯文本 div（`annotate.js:1028`），对比页的操作表也不能点（`annotate_compare.js:174,190`）。
- 参考工具：webKnossos 的版本历史可以只读预览；Photoshop 的历史面板点哪步显示哪步。
- 建议改法：新增 `GET /edits/{n}/mask?z=`，悬停历史行时高亮这一笔的像素，点击居中（复用对比页的 `focusRegion`）。「预览撤到此处」用对比滑块显示，Enter 调第 27 条的 undo-to。
...
- 现在的体验：
  ...
  - Shift+滚轮会翻片（`annotate.js:416-421`）。
...
- 现在的体验：插值只服务于损坏切片，没检测到损坏就直接报错（`interpolate.py:366-368`）。
- 参考工具：webKnossos 在两片上画同一 id 后按 V；TrakEM2 的 Interpolate gaps；Paintera 的形状插值。
- 建议改法：新增 `interpolate_id(block, id, za, zb)`，复用 `_sdf/_tau/claim`，只写中间片的空白像素，整次插值记成一条多片记录，标为待复查。按 I 自动找「上一张画过当前 id 的片」，预览后 Enter 应用。
...
**39. 没有 Z 向断档扫描**（工作量：小）
- 现在的体验：损坏切片有扫描按钮，Z 向断档没有，只能翻片时碰上。
...
**41. 修补预览按整块文件指纹判断过期**（工作量：小）
- 现在的体验：preview 和 apply 比较的是 `revision(block)`（`interpolate.py:373,401-403`），同块任何人在任何一片画一笔，你的预览就作废。SAM 已经改成按片判断（`sam.py:148-152`）。
- 参考工具：只在真正冲突的范围内拒绝写入。
- 建议改法：preview 时记下 z±6 各片的 `slice_rev`，apply 时只比较这几片。
...
- 现在的体验：
  - 点行只调 `setCur`（`annotate.js:970-974`）；列表每次按「当前置顶」整体重排（`annotate.js:960`）。
...
- 现在的体验：z 滑条是一根空白的 range（`annotate.html:71`）；滚轮每累计 40 px 翻一片，不按手势限流（`annotate.js:416-421`）。对比页已经做了手势限流（`annotate_compare.js:535-544`）。
- 参考工具：CVAT 可以跳到下一个有 Issue 的帧；ImageJ 放大后显示缩略导航图。
- 建议改法：滑条下画刻度带，标出有改动、有损坏、有标记的片；加跳到下一个有改动片的快捷键；放大超过 1.5 倍时显示小地图；复用对比页的滚轮限流逻辑。
...
- 现在的体验：
  ...
  - 修补预览持锁插值（`interpolate.py:361-373`）。
...
- **非画笔类操作的结果提示**：修缮、合并、修补、批量删除、SAM 都有（`annotate.js:448,512,646,1005`）。
...
- **P1-2 里「SAM / 插值修补 / 批量删除直接落盘」的说法不对**：
  - SAM 已经是预览→token→应用，按片判断过期，可以取消（`sam.py:65-159`；`annotate.js:727-772`）。
  - 切片修补是预览后再应用，预览分三色并给出像素统计（`interpolate.py:347-408`）。
  - 批量删除和全部撤销都有确认框，写明影响量（`annotate.js:168-202,985-1014`）。
  - 真正直接落盘的是修缮 R、Shift+F 整片、合并的第二下点击。
- **损坏切片插值修补和邻片取色 L**：完整可用（`interpolate.py:57-69,297-328`；`neighbour.py:20-114`），可以作为关键片插值的底座。
