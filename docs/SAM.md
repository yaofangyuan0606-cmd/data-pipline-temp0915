...
3. 自动预览模型评分最高的结果；边界不合适时继续补点，或清除提示重试。框选和点选可以组合。
...
默认勾选“仅填未标注”，预览已经扣除现有有色标签。
要修正已有分割，取消勾选并重新预测后应用。
“贴合膜边界”使用 SAM 区域内的“边界灵敏度”，默认 50%；调整后会重新计算当前预览。
再次点击当前 SAM 工具、切换切片/数据块、离开 SAM 工具、Esc 或清除提示会丢弃当前预览；预览不产生编辑。
推理期间也可取消或调整提示，已取消请求的结果不会重新显示；取消不撤销已保存的标注。
...
```json
{"z":0,"points":[[150,200]],"labels":[1],"box":null,"only_background":true}
```

点使用当前显示坐标 `(x,y)`（x 为列，y 为行；显示切片按 `plane[y,x]` 访问，`plane = arr[:, :, z].T`），标签 1 包含、0 排除；框为 `[x0,y0,x1,y1]`，服务端始终选模型评分最高的结果。
可选 `snap_boundary`（默认 false）启用膜边界后处理，`boundary_sensitivity` 为 0–1（默认 0.5）。网页默认勾选后处理。
...
不指定时执行合成数据的标签、合并、SAM 交互和 3D 浏览器回归；需可用 CUDA 和已安装的 SAM 权重才能执行 GPU 用例。
