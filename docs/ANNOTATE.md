...
**修缮边缘**（R）。点一下标签块，只收它**最外层**跨过黑膜漏出去的部分：在这一片上算局部膜强度，比周围暗得多的像素当成膜，把标签切成几块；离标签边缘超过 10 px 的块（细胞主体）和点击处那块一律留下，细胞里的黑色细胞器（线粒体、囊泡、内质网）都在主体里，不会被动。只有贴着外轮廓的薄片才可能被去掉，而且要看它朝外碰到的是什么：外面大半是亮的胞质（说明是漏进了邻居）就清成背景，外面是暗的（贴着膜）就留下。只收缩不扩张，一笔记录（`kind: refine`，来源归"算法辅助"），Ctrl/⌘+Z 撤销；灵敏度沿用 SAM 栏的「边界灵敏度」滑块（越高越容易把淡的膜当膜）。在 H01 真实数据上，对 48 个本来就贴着膜的细胞只动了0.02% 的像素，对人为外扩 6 px 的漏标收回 98%。
