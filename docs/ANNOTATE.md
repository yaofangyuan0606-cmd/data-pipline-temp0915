...
改半径：按住 Tab 左右移动鼠标（鼠标）；**触控板**上按住键时 macOS 会忽略触控板移动（防误触），所以改成点一下 Tab 松开、再左右移动，单击或再按一次 Tab 结束、Esc 还原；也可以按住 Alt 两指上下滑；[ ] 一格一格调。调半径时不会落笔。橡皮（右键或 E 工具）**只擦当前颜色**的像素——压到邻居身上不会把邻居擦掉（接口 `paint` 的
...
切割、分离和智能填充已移除，按钮、K/X/D 快捷键及对应接口均不再提供。已有标注结果、历史记录、撤销和溯源继续可用，无需迁移数据。

## 当前标签与新建标签

“当前标签”是下一次填充、画笔或 SAM 应用使用的标签，不要求已经出现在当前切片；翻片保留选择。初始显示“未选择”，点背景不会改变选择。
“新建标签”（N）分配块内未使用的 ID，跳过与当前片已有标签及本块新建标签相同的 RGB 颜色，并立即选中。
...
H 平移（或中键拖动 / 空格+拖动） · [ ]、按住 Tab 移动鼠标、点一下 Tab 再移动（触控板）或 Alt+滚轮改画笔半径 · A / Z（也可 W / S、↑ ↓）翻片 · N 新建标签 ·
