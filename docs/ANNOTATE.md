...
**修缮边缘**（R）。点一下标签块，只收它**最外层**：压在黑膜上的那一层、以及跨过黑膜漏到外面的部分。分两步：先在很黑的膜处把标签切开，贴边的薄片如果朝外接触的大多是亮的胞质（漏进了邻居），就清成背景；再从标签外面的黑膜出发，把标签最外 8 px 里顺着暗像素直接够得到的那层剥掉（只剥表层，顺着贴边细胞器钻进去的不剥），剥开后断下来的、漏进邻居的小碎片一起去掉。细胞里的黑色细胞器（线粒体、囊泡）不管贴不贴边都留下：粗得不像膜、又大半落在标签里的暗块受保护，离外沿 8 px 以外的像素永远不动。只收缩不扩张，一笔记录（`kind: refine`，来源归"算法辅助"），Ctrl/⌘+Z 撤销；灵敏度沿用 SAM 栏的「边界灵敏度」滑块（越高越容易把淡的膜当膜）。在 H01 演示块上的评测：108 处人为画出、确实跨过黑膜的漏标，膜上和膜外的部分平均收回 91%（中位数 100%）；38 个原本贴着膜的细胞平均只改 1.9% 的像素，基本是 c3 标签本来就压在膜上的那条边。
