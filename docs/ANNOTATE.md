...
按住 Tab 左右拖动改半径；橡皮（右键或 E 工具）**只擦当前颜色**的像素——压到邻居身上不会把邻居擦掉（接口 `paint` 的
`only_id`），所以擦之前得先拾取要擦的颜色。平移改用中键、空格+拖动或 H。

**修缮边缘**（R）。点一下标签块：算这一片的膜图，标签里不在膜上的像素分成连通块，只留点击处那一块，其余（跨过黑膜溢到
邻居身上的部分）连同它那半边膜一起清成背景；膜上的像素归离它最近的那块，细胞里的线粒体、噪点跟着留下。只收缩不扩张，
一笔记录（`kind: refine`，来源归"算法辅助"），Ctrl/⌘+Z 撤销；灵敏度沿用 SAM 栏的「边界灵敏度」滑块。
...
H 平移（或中键拖动 / 空格+拖动） · [ ] 或按住 Tab 拖动改画笔半径 · A / Z 或 ↑ / ↓ 翻片 · 滚轮缩放 · N 新建标签 ·
