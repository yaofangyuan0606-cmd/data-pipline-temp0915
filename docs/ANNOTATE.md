...
**试用模式** `EMQC_AUTH_OPEN=1`：登录页照常，但不校验密码——填个名字就进（中文名也行，会当显示名，登录名由它的哈希生成），
账号不存在就自动建成审核员，停用的账号仍然进不来。账号还没发齐、内部同事还在整的阶段用；之后去掉这个变量、管理员把
自动建出来的账号重置一下密码，就切回正式模式，期间的改动记录不受影响。
